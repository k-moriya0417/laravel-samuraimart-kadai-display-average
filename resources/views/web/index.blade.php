                 ...
                 <div class="row">
                     <div class="col-12">
                         <p class="samuraimart-product-label mt-2">
                             ...
                             @php
                                $totalScore = 0;
                                $reviewCount = count($recommend_product->reviews);
                                foreach ($recommend_product->reviews as $review) {
                                    $totalScore += $review->score;
                                }
                                $averageScore = $reviewCount > 0 ? round($totalScore / $reviewCount, 1) : 0;
                                $starRate = round($averageScore * 2, 0) / 2;
                            @endphp
                            <span class="star-wrapper">
                                <span class="star-rating" data-rate="{{$starRate}}"></span>
                            </span>
                            <span>{{$averageScore}}</span><br>
                     ...
                     <div class="row">
                         <div class="col-12">
                             <p class="samuraimart-product-label mt-2">
                                 ...
                                 @php
                                    $totalScore = 0;
                                    $reviewCount = count($recently_product->reviews);
                                    foreach ($recently_product->reviews as $review) {
                                        $totalScore += $review->score;
                                    }
                                    $averageScore = $reviewCount > 0 ? round($totalScore / $reviewCount, 1) : 0;
                                    $starRate = round($averageScore * 2, 0) / 2;
